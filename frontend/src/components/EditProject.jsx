...
export const EditProject = ({ onClose , project }) => {
  const projectID = project._id;
  const queryClient = useQueryClient();
  const[techStack,setTechStack] = useState(project.Technology);
  const[keyPoints,SetKeyPoints] = useState(project.KeyPoints);
  ...
  const { mutate : updateProject , isPending , isError , error } = useMutation({
    mutationFn : async(updateProject)=> {
      try {
        const res = await fetch(`${baseURL}/api/project/updateProject/${projectID}`,{
          method : "PATCH",
          credentials : "include",
          body : updateProject
        });
    ...
    onError: (error) => {
      toast.error("Unable to Edit the Project");
  ...
  const updateProjectOnSubmit = async (e) => {
    e.preventDefault();
    ...
    <div className="add-new-project-wrapper">
      <div className="add-new-project-container p-3">
        <div className="add-new-project-container-header d-flex align-items-center justify-content-between">
            <p>Edit New Project</p>
            <button className="border-0 bg-transparent px-3 rounded-3" onClick={onClose} >X</button>
        </div>
        <form onSubmit={updateProjectOnSubmit} className="row" id="add-new-project-form">
          <div className="col-lg-6 col-12 mb-2">
            <div className="form-group mt-2">
              <label htmlFor="name" className="mb-1">Enter the Name</label>
              <input className="form-control" id="name" value={name} onChange={(e)=>setName(e.target.value)} type="text" placeholder="Enter the Name of the Project" />
            </div>
            <div className="form-group mt-3">
               <label htmlFor="short-discription" className="mb-1">Enter the short Discription</label>
              <input className="form-control" id="short-discription" value={shortDiscription} onChange={(e)=>setshortDiscription(e.target.value)} type="text" placeholder="Enter the short Discription" />
            </div>
            <div className="form-group mt-3">
              <label htmlFor="long-discription" className="mb-1">Enter the Long Discription</label>
              <input className="form-control" id="long-discription" value={longDiscription} onChange={(e)=>setLongDiscription(e.target.value)} type="text" placeholder="Enter the Full Discription" />
            </div>
            <div className="form-group mt-3">
            ...
            <div className="from-image-input-container d-flex mt-3">
                <div className="w-50">
                  <label htmlFor="main-img" className="mb-2"> Select Main Image</label>
                  <input type="file" name="main-img" id="main-img" onChange={(e) => { const file = e.target.files?.[0]; if(file) setMainImg(file) }}  />
                </div>
                <div className="w-50">
                  <label htmlFor="hero-img" className="mb-2"> Select Hero Image</label>
                  <input type="file" name="hero-img" id="hero-img" onChange={(e) => { const file = e.target.files?.[0]; if(file) setHeroImg(file) }}  />
                </div>
            </div>
          </div>
          ...
          <div className="col-12 mt-2"><button type="submit" className="w-100 py-2 border-0 add-new-project-form-submit-button" disabled={isPending}>{isPending ? "Updating..." : "Update"}</button></div>
        </form>
      </div>
    </div>
  )
